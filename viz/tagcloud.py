...
def drawCloud(tags):
   ...
   common_words = open('viz/common-words.txt').read().split()
   ...
   if(not len(tags)):
      return Image.new("RGB",(width,height),"#ddd") 
   ...
   arial7 = ImageFont.truetype(fontPath,7) 
   arial10 = ImageFont.truetype(fontPath,10) 
   ...
   font_classes = [arial7, arial10, arial18, arial28, arial36]
   ...
   for (t,w) in tags:
       ...
       if (x + word_w) < width:
           ...
           if c > max_font_index:
               max_font_index = c
   if not lines: lines=[current_line]
   image_width = 0
   image_height = 0
   lines_adjusted = []
   for line in lines:
   ...
   for line in lines_adjusted:
       for word_data in line:
           ...
           x,y = word_data[2]
           font = word_data[4]
           draw.text((x,y),unicode(word,'UTF-8'),font=font,fill="black") 
